...
export class UIManager {
	...
	private generateHtmlHeader(fontSize: number, tabHeight: number): string {
		...
		<head>
			...
			.vscode-tooltip {
			  ...
			  opacity: 0;
			  visibility: hidden;
			  /* Solo transición de aparición, no de desaparición */
			...
			.vscode-tooltip.hiding {
			  /* Desaparición instantánea sin transición */
			  opacity: 0 !important;
			...
			.tab { 
				...
				/* Transición solo para posicionamiento vertical */
				transition: transform 0.2s ease-out;
			}
			.tab.closing {
				/* La pestaña que se cierra no tiene transición y desaparece inmediatamente */
				opacity: 0;
				transform: scaleY(0);
				height: 0;
				min-height: 0;
	...
	function hideTooltip() {
		...
		if (activeTooltip) {
			...
			// Remover la clase hiding después de un frame para permitir futuras animaciones
			setTimeout(() => {
				if (activeTooltip) {
					activeTooltip.classList.remove('hiding');
				}
			}, 16); // ~1 frame
		}
	}
